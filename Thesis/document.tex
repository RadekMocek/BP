...
\begin{document}
	...
	K odeslané zprávě je přidána trojice tlačítek \textit{Potvrdit}, \textit{Upravit} a \textit{Smazat}. Aby mohl být ke zprávě přidán jakýkoliv interaktivní prvek, musí být nejprve vytvořen tzv. view (pohled). View slouží jako kontejner pro všechny interaktivní prvky a ke zprávě je přidán přes stejnojmenný parametr (viz obrázek \ref{_tag_img_discordpy}). % Pro složitější logiku je možné z view dědit, uchovat vnitřní stav, tlačítka mají odkaz na svůj view a mohou v něm volat metody atd.
	
	% WEBHOOKS TOKEN EXPIRATION AFTER 15 MINUTES ?
