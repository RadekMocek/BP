...
\begin{document}
	...
	Nakonec byl zvolen přístup, kde si každý uživatel může provozovat vlastní instanci navrhovaného bota. Soubory s tématy jsou pak nakopírovány do vyhrazeného adresáře, odkud mohou být snadno spravovány. %TODO
	...
	K odeslané zprávě je přidána trojice tlačítek \textit{Potvrdit}, \textit{Upravit} a \textit{Smazat}. Aby mohl být ke zprávě přidán jakýkoliv interaktivní prvek, musí být nejprve vytvořen tzv. view (pohled). View slouží jako kontejner pro všechny interaktivní prvky a ke zprávě je přidán přes stejnojmenný parametr (viz obrázek \ref{_tag_img_discordpy}). Pro definici složitější view logiky je možné vytvořit třídu, která dědí z obecného view. Do této třídy se pak přidají atributy pro uchování určitého stavu a metody pro jeho změnu. Každý interaktivní prvek má referenci na své view, díky tomu v něm lze např. po stisknutí tlačítka volat určitou metodu.
	
	V tomto případě byla vytvořena třída, která dědí z view a navíc uchovává referenci na zprávu, ke které je přiřazena, a na uživatele, který zadal příkaz vyvolávající tuto zprávu. Odkaz na uživatele se využívá pro kontrolu oprávnění a odkaz na původní zprávu je potřeba pro funkci tlačítka \textit{Smazat}. Jednotlivá tlačítka také dědí z obecné třídy, u které primárně přepisují metodu \textit{callback} volanou jejich po stisknutí. Tlačítko pro úpravu matematického výrazu navíc musí mít uložený původní uživatelský vstup, který je předvyplněn do vyvolaného vyskakovacího okna, a barevné schéma, které bylo pro dané vykreslení použito. Vyskakovací okno po svém potvrzení volá stejnou funkci pro vykreslení jako příkaz \verb*|/render|, jen místo nové zprávy upraví tu původní. Zvolené barevné schéma se uchovává v proměnné, aby se nemuselo při každé úpravě výrazu znovu zjišťovat.
	
	% TODO Tato sekce vyžaduje revizi ------------------
	...
	% Poslední odstavce jsou dojezd; zkontrolovat :)
	% TODO ---------------------------------------------
